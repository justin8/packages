...
. /etc/conf.d/ftb-direwolf20 || exit 1
...
case $1 in
	start)
		...
		then
			...
			screen -dmS "$FTBUSER" su "$FTBUSER" -c "java -Xmx${MAXHEAP} -Xms${MINHEAP} -XX:PermSize=${PERMSIZE} -XX:ParallelGCThreads=${GCTHREADS} -XX:+UseParNewGC -XX:+UseConcMarkSweepGC -jar ${_SRVDIR}/${SERVERJAR} nogui"
		fi
		;;
	stop)
		if screen -ls|grep $FTBUSER
